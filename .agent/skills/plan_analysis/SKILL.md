...
- **Robustness Check**: Look for missing error states, loading timeouts, and fallback UI.
...
1.  **Mandatory Report Output**: The result of this skill MUST always be a comprehensive analysis report (artifact). Do not just summarize in chat.
2.  **Plan Creation Trigger**: After the analysis is approved by the user, you MUST create a new `implementation_plan.md` (or update the existing one) that incorporates the findings.
3.  **Strict Blocking**: **NEVER start implementation code changes** based on your analysis until the user has explicitly approved the new `implementation_plan.md`.
    *   **Workflow**: Analyze → Report → User Approval → Create/Update Plan → User Approval → Implement.
...
1. **Present Report**: Notify user of the analysis artifact.
2. **WAIT for Approval**: Do not proceed until user reviews the analysis.
3. **Create Implementation Plan**: Once analysis is accepted, create `implementation_plan.md`.
4. **WAIT for Plan Approval**: Do not write code until the plan is approved.
5. **Begin Execution**: Only now can you switch to EXECUTION mode.
...
## 🔗 Skill Chaining (Integration)

**Where to go next?**

1.  **If the plan is Approved**:
    *   👉 Go to **`break_down_tasks`** to convert the `implementation_plan.md` into atomic `task.md` items.
2.  **If the plan has Architecture/Design issues**:
    *   👉 Go to **`system_architecture`** or **`update_design_system`** to resolve structural problems before breakdown.
3.  **If the plan involves Refactoring**:
    *   👉 Go to **`componentize_feature`** for specific refactoring strategies.
